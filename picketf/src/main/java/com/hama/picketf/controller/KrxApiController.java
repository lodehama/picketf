...
public class KrxApiController {
  ...
  public ResponseEntity<?> getOne(@PathVariable String isuCd,
    ...
    try {
      KrxEtf dto = krxApiService.getEtf(isuCd, basDd);
  ...
  public ResponseEntity<?> getMany(@RequestParam String codes,
    ...
    try {
      List<String> list = Arrays.stream(codes.split(","))
          ...
          .filter(s -> !s.isBlank())
          .toList();
      Map<String, KrxEtf> map = krxApiService.getEtfs(list, basDd);
  ...
  // 프론트 분리 테스트용
  @GetMapping("/_dummy")
  public Map<String, KrxEtf> dummy() {
    return Map.of("379800", new KrxEtf(), "433330", new KrxEtf());
  }
}
